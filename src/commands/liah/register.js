module.exports.execute = async (client, message, locale, embed, tools, knex, props, data) => {
  if ((await knex.select("*").from("users").where({ id: message.author.id })).length > 0) {
    ...
    if (data.register.includes(message.author.id)) return message.reply(locale.error.process)
    embed.setDescription(locale.commands.register.yet)
    embed.addField(locale.commands.register.register, locale.commands.register.start)
    embed.addField(
